...
@TeleOp(name="Pedro Vision TeleOp")
public class RobotPedroVis extends OpMode {

    private Follower pedroFollowsYou;
    private AprilTags apriltags;
    private PoseHistory poseHist;

    @Override
    public void init(){
        pedroFollowsYou = Constants.createFollower(hardwareMap);
        pedroFollowsYou.setStartingPose(new Pose(0,0,0));
        apriltags = new AprilTags(hardwareMap);
        Drawing.init();
    }
    @Override
    public void start(){
        pedroFollowsYou.startTeleOpDrive();
        telemetry.setMsTransmissionInterval(50);
        poseHist = pedroFollowsYou.getPoseHistory();
    }
    @Override
    public void loop(){
        ...
        if (!apriltags.getDetections().isEmpty()) {
            // we only want to update the position when we know that we have a different thing happening
            Pose tagPose = apriltags.getPedroPose();
            pedroFollowsYou.setPose(tagPose);
        }

        Drawing.drawRobot(pedroFollowsYou.getPose());
        Drawing.drawPoseHistory(poseHist);
        Drawing.sendPacket();

        pedroFollowsYou.setTeleOpDrive(
